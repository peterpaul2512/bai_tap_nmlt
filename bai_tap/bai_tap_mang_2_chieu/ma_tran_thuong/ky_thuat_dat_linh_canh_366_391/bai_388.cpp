// Bài 388: Liệt kê các dòng có nhiều số nguyên tố nhất
#include <iostream>
using namespace std;

void inputMatrix(int matrix[100][100], int &rows, int &cols);

void printMatrix(int matrix[100][100], int rows, int cols);

bool isPrime(int n);

void listPrimeRow(int matrix[100][100], int rows, int cols);

int main() {
    int matrix[100][100];
    int rows, cols;

    inputMatrix(matrix, rows, cols);
    printMatrix(matrix, rows, cols);

    listPrimeRow(matrix, rows, cols);

    return 0;
}

void inputMatrix(int matrix[100][100], int &rows, int &cols) {
    cout << "Enter matrix rows: ";
    cin >> rows;
    cout << "Enter matrix cols: ";
    cin >> cols;

    if (rows <= 0 || cols <= 0 || rows > 100 || cols > 100) {
        cout << "Invalid input" << endl;
        return;
    }

    cout << "Enter matrix elements: " << endl;
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            cout << "Enter matrix[" << i << "][" << j << "]: ";
            cin >> matrix[i][j];
        }
    }
}

void printMatrix(int matrix[100][100], int rows, int cols) {
    cout << "Matrix elements: " << endl;
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < cols; j++) {
            cout << matrix[i][j] << "\t"; // thêm tab
        }
        cout << endl;
    }
}

bool isPrime(int n) {
    if (n < 2) {
        return false;
    }
    for (int i = 2; i * i <= n; i++) {
        if (n % i == 0) {
            return false;
        }
    }
    return true;
}

void listPrimeRow(int matrix[100][100], int rows, int cols) {
    int maxPrimeCount = 0;
    for (int i = 0; i < rows; i++) {
        int primeCount = 0;
        for (int j = 0; j < cols; j++) {
            if (isPrime(matrix[i][j])) {
                primeCount++;
            }
        }
        if (primeCount > maxPrimeCount) {
            maxPrimeCount = primeCount;
        }
    }
    cout << "Rows with the most prime numbers: " << endl;
    for (int i = 0; i < rows; i++) {
        int primeCount = 0;
        for (int j = 0; j < cols; j++) {
            if (isPrime(matrix[i][j])) {
                primeCount++;
            }
        }
        if (primeCount == maxPrimeCount) {
            cout << "Row " << i << " : " << maxPrimeCount << endl;
        }
    }
}
